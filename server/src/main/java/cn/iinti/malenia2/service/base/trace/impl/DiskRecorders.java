package cn.iinti.malenia2.service.base.trace.impl;
...
public class DiskRecorders {
    ...
    public DiskRecorder acquireRecorder(String sessionId, String scope, boolean debug) {
        return this.wheelSlotFilter.acquireRecorder(debug) ?
                new DiskRecorderImpl(sessionId, scope) : nopDiskRecorder;
    }
    ...
    private class DiskRecorderImpl extends DiskRecorder {
        ...
        private DiskRecorderImpl(String sessionId, String scope) {
            this.sessionId = sessionId;
            this.scope = StringUtils.defaultIfBlank(scope, "default");
        }

        @Override
        void recordBatchEvent(Collection<String> msgLines) {
            thread.execute(() -> {
                MDC.put("scope", tag + "_" + scope);
