...
public class OutboundOperator {
    ...
    public static void connectToOutbound(Session session, long sessionHash, String tag, ActiveProxyIp.ActivityProxyIpBindObserver observer, ValueCallback<Channel> callback) {
        session.getWrapperProduct().connectToOutbound(session, sessionHash, tag, observer, callback);
    }
}
