...
import cn.iinti.malenia2.api.ip.resource.IpAuthBuilder;
...
public class HandShakerSocks5 extends AbstractUpstreamHandShaker {
    ...
    public void doHandShark() {
        ...
        if (!activeProxyIp.skipAuth(session.getSessionParam())) {
    ...
    private void doConnectToUpstream() {
        ...
        SocksCmdRequest socksCmdRequest = new SocksCmdRequest(SocksCmdType.CONNECT,
                IpResourceHelper.isIpV4(ipAndPort.getIp()) ? SocksAddressType.IPv4 : SocksAddressType.DOMAIN,
    ...
    private void doAuth() {
        IpAuthBuilder.AuthUser authenticationInfo = activeProxyIp.buildAuthenticationInfo(session.getSessionParam(), recorder);
        if (authenticationInfo == null) {
            emitFailed("the upstream server need auth,but no auth configured for this ip source : " + activeProxyIp.getIpPool().getWrapperIpSource().getSourceKey(), false);
            return;
        }
        recorder.recordMosaicMsg(() -> "send socks5 auth with user: " + authenticationInfo);
        pipeline.addFirst(new SocksAuthResponseDecoder());
        SocksAuthRequest socksAuthRequest = new SocksAuthRequest(authenticationInfo.getUserName(), authenticationInfo.getPassword());
