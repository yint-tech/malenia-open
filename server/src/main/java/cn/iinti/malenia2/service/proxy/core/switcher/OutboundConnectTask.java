...
import cn.iinti.malenia2.service.base.safethread.ParallelExecutor;
...
import cn.iinti.malenia2.service.proxy.core.outbound.handshark.AbstractUpstreamHandShaker;
import cn.iinti.malenia2.service.proxy.core.outbound.handshark.Protocol;
import cn.iinti.malenia2.service.proxy.core.outbound.handshark.ProtocolManager;
...
public class OutboundConnectTask implements ActiveProxyIp.ActivityProxyIpBindObserver {
    ...
    private OutboundConnectTask(Session session, UpstreamHandSharkCallback callback) {
        ...
        this.tags = Tags.of("productId", session.getWrapperProduct().getProductId(), "inboundProtocol", session.getInboundProtocol().name());
        this.callback = new EatErrorFilter(callback, recorder);
    }

    private void doStart() {
        if (session.isDirectSend()) {
            directConnect();
        } else {
            proxyForward(null);
        }
    ...
    private void directConnect() {
        // 如果是直连，那么不经过上游转发，这里直接创建到真实服务器的连接即可
        // 请注意如果是socks代理，可能存在使用udp，但是我们现在还没有支持udp转发
        // 如果是udp那么这里需要调整下代码
        OutboundOperator.connectToServer(session.getConnectTarget().getIp(), session.getConnectTarget().getPort(), value -> {
            if (!value.isSuccess()) {
                callback.onHandSharkError(value.e);
                return;
            }
            //直接连接模式没有上游握手
            callback.onHandSharkFinished(value.v, null);
        });
    }

    private void proxyForward(Throwable throwable) {
        if (failoverCount >= session.getWrapperOrder().getMaxFailoverCount()) {
            recorder.recordEvent(() -> "failoverCount count: " + failoverCount + " maxCount:" + session.getWrapperOrder().getMaxFailoverCount());
            if (throwable == null) {
        ...
        if (failoverCount == 1) {
            ...
            // 尽可能保证隧道映射不变
            wrapperProduct.fetchCachedSession(session, value -> {
                if (value.isSuccess()) {
        ...
        failoverConnect(channelCallback, sessionHash, wrapperProduct);
    }

    @Override
    public void onBind(ActiveProxyIp activeProxyIp) {
        resolvedIpSource = activeProxyIp.getIpPool().getWrapperIpSource().getSourceKey();
    }
    ...
    private void failoverConnect(ValueCallback<Channel> channelCallback, long sessionHash, WrapperProduct wrapperProduct) {
        // 不是第一次，那么同时使用多个资源创建ip，谁成功以谁为准
        ParallelExecutor<FailoverTaskHolder> executor = new ParallelExecutor<>(wrapperProduct.getProductThread(), parallelSize,
        ...
        for (int i = 0; i < parallelSize; i++) {
            ...
            OutboundOperator.connectToOutbound(session, newHash, tag, this,
                    value -> {
                        if (value.isSuccess()) {
                            executor.onReceiveValue(ValueCallback.Value.success(new FailoverTaskHolder(newHash, index, value.v, tag)));
                        } else {
    ...
    private ValueCallback<Channel> makeChannelFinishedEvent() {
        return channelValue -> {
            Monitor.counter(WrapperProduct.METRIC_PREFIX + "connect", Tags.concat(tags,
                    "sourceKey", resolvedIpSource,
                    "success", String.valueOf(channelValue.isSuccess())
            )).increment();
            Monitor.counter(WrapperProduct.METRIC_PREFIX + "connect4User_" + session.getWrapperUser().getUserName(), Tags.concat(tags,
                    "success", String.valueOf(channelValue.isSuccess())
            )).increment();
            ...
            List<Protocol> supportProtocolList = activeProxyIp.getIpPool().getWrapperIpSource().getSupportProtocolList();
            ...
            AbstractUpstreamHandShaker handShaker = ProtocolManager.createHandShaker(outboundProtocol, session, channel, activeProxyIp, value -> {
                Monitor.counter(WrapperProduct.METRIC_PREFIX + "handshark", Tags.concat(tags,
                        "sourceKey", resolvedIpSource,
                        "success", String.valueOf(value.isSuccess())
                )).increment();
                if (value.isSuccess()) {
                    recorder.recordEvent(() -> "HandShark success");
                    callback.onHandSharkFinished(channel, outboundProtocol);

                    session.getWrapperProduct().markSessionUse(session, activeProxyIp);
                    return;
                }
                channel.close();
                if (value.v) {
