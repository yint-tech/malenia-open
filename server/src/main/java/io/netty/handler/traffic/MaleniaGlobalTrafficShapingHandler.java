...
/**
 * @author yint 增加malenia框架的recorder，在限流发生时，打印日志
 *
 ...
 */
@Sharable
public class MaleniaGlobalTrafficShapingHandler extends AbstractTrafficShapingHandler {
    /**
     ...
     */
    public MaleniaGlobalTrafficShapingHandler(ScheduledExecutorService executor, long writeLimit, long readLimit,
     ...
     *                      channels or 0 if no stats are to be computed.
     */
    public MaleniaGlobalTrafficShapingHandler(ScheduledExecutorService executor, long writeLimit,
     ...
     *                      channels or 0 if no stats are to be computed.
     */
    public MaleniaGlobalTrafficShapingHandler(ScheduledExecutorService executor, long checkInterval) {
        super(checkInterval);
        createGlobalTrafficCounter(executor);
    }
     ...
     * @param executor the {@link ScheduledExecutorService} to use for the {@link TrafficCounter}.
     */
    public MaleniaGlobalTrafficShapingHandler(EventExecutor executor) {
        createGlobalTrafficCounter(executor);
    }
     ...
     *
     * @see AbstractTrafficShapingHandler#MINIMAL_WAIT
     */
    ...
    long checkWaitReadTime(final ChannelHandlerContext ctx, long wait, final long now) {
        ...
        if (wait >= MINIMAL_WAIT) {
            Session session = Session.get(ctx.channel());
            if (session != null) {
                session.getRecorder().recordEvent(() -> "this session has hint bandwidth limit: " + trafficCounter);
            }
        }
        return wait;
    }
